...
class Critic(nn.Module):
    def __init__(self, obs_dim, action_dim, hidden_dim, init_w=1e-3):
        ...
        self.q_dis = nn.Linear(hidden_dim*2, 200)
    ...
    def forward(self, obs, action):
        ...
        qdis = self.q_dis(torch.cat([q1, q2], dim=-1))
        qdis = F.softmax(qdis, dim=-1)
...
class CDSAgent(Agent):
    ...
    # 定义一个辅助方法，用于重复应用Critic网络以计算多个动作的Q值
    def _repeated_critic_apply(self, obs, actions):
        ...
        Q1, Q2, _ = self.critic(repeated_obs, reshaped_actions)
        Q1 = Q1.reshape((n_samples, batch_size, 1))
        Q2 = Q2.reshape((n_samples, batch_size, 1))

        return Q1, Q2
    
    def fast_target_Q(self, target_V, reward, discount, device):
        batch_size, num_actions = target_V.shape
        
        # 创建一个 [1024, 200] 的索引矩阵，表示每个位置对应的动作值
        action_values = torch.arange(-100, 100, dtype=torch.float32).unsqueeze(0).repeat(batch_size, 1).to(device)
        
        # 计算每个位置的目标值
        target_values = reward + discount * action_values
        
        # 将目标值限制在 [-100, 99] 的范围内
        target_values = torch.clamp(target_values, -100, 99)
        
        # 计算每个位置的下限和上限索引
        floor_indices = torch.floor(target_values).long() + 100
        ceil_indices = torch.ceil(target_values).long() + 100
        
        # 计算每个位置的权重
        floor_weights = 1 - (target_values - torch.floor(target_values))
        ceil_weights = 1 - (torch.ceil(target_values) - target_values)
        
        # 创建目标Q值矩阵
        target_Q = torch.zeros(batch_size, 200, dtype=torch.float32).to(device)
        
        # 使用scatter_add_更新目标Q值矩阵
        target_Q.scatter_add_(1, floor_indices, floor_weights * target_V)
        target_Q.scatter_add_(1, ceil_indices, ceil_weights * target_V)
        
        return target_Q

    # 定义更新Critic网络的方法，接收观察状态、动作、奖励、折扣因子、下一个观察状态和当前步骤作为参数。
    def update_critic(self, obs, action, reward, discount, next_obs, step):
        ...
        with torch.no_grad():
            ...
            _, _, target_V = self.critic_target(next_obs, sampled_next_action)  # (1024,200)
            # print(target_V.type)
            # target_Q = torch.zeros(1024, 200)   # (1024,200)
            # for i in range(1024):
            #     for j in range(200):
            #         v = reward + discount * (j-100)
            #         if v>99:
            #             v=99
            #         if v<-100:
            #             v=-100
            #         p = math.ceil(v)
            #         q = math.floor(v)
            #         target_Q[i,p+100] += (1-p+v) * target_V[i,j]
            #         target_Q[i,q+100] += (1-v+q) * target_V[i,j]
            
            target_Q = self.fast_target_Q(target_V, reward, discount, self.device)
                    


        Q1, Q2, Qdis = self.critic(obs, action)
        # print(Qdis.type)
        critic_loss = F.cross_entropy(Qdis, target_Q)   # 标量
    ...
    def update_actor(self, obs, action, step):
        ...
        # optimize actor
        Q1, Q2, _ = self.critic(obs, sampled_action)     # (1024, 1)
    ...
    def conservative_data_share(self, batch_main, batch_share):
        ...
        with torch.no_grad():
            conservative_q_value, _, _ = self.critic(obs, action)          # (5120, 1)
            conservative_q_value_main, _, _ = self.critic(obs_m, action_m)
        
        conservative_q_value = conservative_q_value.squeeze().detach().cpu().numpy()
        conservative_q_value_main = conservative_q_value_main.squeeze().detach().cpu().numpy()

        percentile_10 = np.percentile(conservative_q_value_main, 10, axis=0)
        top_index = np.where(conservative_q_value > percentile_10)[0]
        if top_index.shape[0] < obs_m.shape[0]:
            percentile_10_share = np.percentile(conservative_q_value, 10, axis=0)
            top_index = np.where(conservative_q_value >= percentile_10_share)[0]
